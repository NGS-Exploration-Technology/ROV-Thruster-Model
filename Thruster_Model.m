...
if Thruster_Config.RH_prop
    ...
    %d_n = -kn1*n0-kn2*n0*abs(n0)+kq*Q0+kv*n_command; % RH prop
    %d_n = -kn1*n0-kn2*n0*abs(n0)+kv*n_command; % RH prop
    d_n = kv*(n_command-n0); % RH prop
else
    %d_n = -kn1*n0-kn2*n0*abs(n0)-kq*Q0+kv*Throttle; % LH prop
    %d_n = -kn1*n0-kn2*n0*abs(n0)-kq*Q0+kv*n_command; % LH prop
    %d_n = -kn1*n0-kn2*n0*abs(n0)+kv*n_command; % LH prop
