...
[cTn,cTnv,cTv,cQn,cQnv,cQv] = Generate_Thrust_Curves_22();
lenp3 = 4300:13500;
lenp2 = 3450:12500;
lenm2 = 4550:13500;
lenm3 = 4500:14000;
...
% 
% J = -1:.001:1;
% KT1 = (-Thruster_Config.cTnv*J+Thruster_Config.cTn)/(rho*D^4);
% KT2 = (Thruster_Config.cTn-cTnv*J/3-cTv*J.*abs(J)/1000)/(rho*D^4);
% figure
% plot(J,KT2,J,KT1)
% grid
...
v = {vp3,vp2,vm2,vm3};
constants = {kv1,kv2,dv1,dv2,kV,kVV,kTV,cTn,cTnv,cTv,cQn,cQnv,cQv,dt,dtv};

%define Fitness Function
f = @(x)fitness_fcn_thruster_curve_22(x,t,tv,n,v,u,constants);

[x fval exitflag opt_output] = fminsearch(f,[kn 1800 1 60 .01], options)
...
for i = 2:length(n_fitp3)
    Q_fitp3(i-1) = cQn*n_fitp3(i-1)*abs(n_fitp3(i-1))-cQnv*v_fitp3(idx-1)*abs(n_fitp3(i-1))-cQv*v_fitp3(idx-1)*abs(v_fitp3(idx-1));
    d_n = -kn*n_fitp3(i-1)-kq*Q_fitp3(i-1)+kv2*(up3(i-1)-dv2);
    n_fitp3(i) = n_fitp3(i-1)+d_n*dt;
    
    if tvp3(idx-1) <= tp3(i-1)
        T_fitp3(idx-1) = cTn*n_fitp3(i-1)*abs(n_fitp3(i-1))-cTnv*v_fitp3(idx-1)*abs(n_fitp3(i-1))-cTv*v_fitp3(idx-1)*abs(v_fitp3(idx-1));
...
for i = 2:length(n_fitp2)
    Q_fitp2(i-1) = cQn*n_fitp2(i-1)*abs(n_fitp2(i-1))-cQnv*v_fitp2(idx-1)*abs(n_fitp2(i-1))-cQv*v_fitp2(idx-1)*abs(v_fitp2(idx-1));
    d_n = -kn*n_fitp2(i-1)-kq*Q_fitp2(i-1)+kv2*(up2(i-1)-dv2);
    n_fitp2(i) = n_fitp2(i-1)+d_n*dt;
    
    if tvp2(idx-1) <= tp2(i-1)
        T_fitp2(idx-1) = cTn*n_fitp2(i-1)*abs(n_fitp2(i-1))-cTnv*v_fitp2(idx-1)*abs(n_fitp2(i-1))-cTv*v_fitp2(idx-1)*abs(v_fitp2(idx-1));
...
for i = 2:length(n_fitm2)
    Q_fitm2(i-1) = cQn*n_fitm2(i-1)*abs(n_fitm2(i-1))-cQnv*v_fitm2(idx-1)*abs(n_fitm2(i-1))-cQv*v_fitm2(idx-1)*abs(v_fitm2(idx-1));
    d_n = -kn*n_fitm2(i-1)-kq*Q_fitm2(i-1)+kv1*(um2(i-1)-dv1);
    n_fitm2(i) = n_fitm2(i-1)+d_n*dt;
    
    if tvm2(idx-1) <= tm2(i-1)
        T_fitm2(idx-1) = cTn*n_fitm2(i-1)*abs(n_fitm2(i-1))-cTnv*v_fitm2(idx-1)*abs(n_fitm2(i-1))-cTv*v_fitm2(idx-1)*abs(v_fitm2(idx-1));
...
for i = 2:length(n_fitm3)
    Q_fitm3(i-1) = cQn*n_fitm3(i-1)*abs(n_fitm3(i-1))-cQnv*v_fitm3(idx-1)*abs(n_fitm3(i-1))-cQv*v_fitm3(idx-1)*abs(v_fitm3(idx-1));
    d_n = -kn*n_fitm3(i-1)-kq*Q_fitm3(i-1)+kv1*(um3(i-1)-dv1);
    n_fitm3(i) = n_fitm3(i-1)+d_n*dt;
    
    if tvm3(idx-1) <= tm3(i-1)
        T_fitm3(idx-1) = cTn*n_fitm3(i-1)*abs(n_fitm3(i-1))-cTnv*v_fitm3(idx-1)*abs(n_fitm3(i-1))-cTv*v_fitm3(idx-1)*abs(v_fitm3(idx-1));
