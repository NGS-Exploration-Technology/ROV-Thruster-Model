function loss = fitness_fcn_thruster_curve_22(x,t,tv,n,v,u,constants)
...
for i = 2:length(n_fitp3)
    Q = cQn*n_fitp3(i-1)*abs(n_fitp3(i-1))-cQnv*v_fitp3(idx-1)*abs(n_fitp3(i-1))-cQv*v_fitp3(idx-1)*abs(v_fitp3(idx-1));
    d_n = -kn*n_fitp3(i-1)-kq*Q+kv2*(up3(i-1)-dv2);
    n_fitp3(i) = n_fitp3(i-1)+d_n*dt;
    
    if tvp3(idx-1) <= tp3(i-1)
        T = cTn*n_fitp3(i-1)*abs(n_fitp3(i-1))-cTnv*v_fitp3(idx-1)*abs(n_fitp3(i-1))-cTv*v_fitp3(idx-1)*abs(v_fitp3(idx-1));
...
for i = 2:length(n_fitp2)
    Q = cQn*n_fitp2(i-1)*abs(n_fitp2(i-1))-cQnv*v_fitp2(idx-1)*abs(n_fitp2(i-1))-cQv*v_fitp2(idx-1)*abs(v_fitp2(idx-1));
    d_n = -kn*n_fitp2(i-1)-kq*Q+kv2*(up2(i-1)-dv2);
    n_fitp2(i) = n_fitp2(i-1)+d_n*dt;
    
    if tvp2(idx-1) <= tp2(i-1)
        T = cTn*n_fitp2(i-1)*abs(n_fitp2(i-1))-cTnv*v_fitp2(idx-1)*abs(n_fitp2(i-1))-cTv*v_fitp2(idx-1)*abs(v_fitp2(idx-1));
...
for i = 2:length(n_fitm2)
    Q = cQn*n_fitm2(i-1)*abs(n_fitm2(i-1))-cQnv*v_fitm2(idx-1)*abs(n_fitm2(i-1))-cQv*v_fitm2(idx-1)*abs(v_fitm2(idx-1));
    d_n = -kn*n_fitm2(i-1)-kq*Q+kv1*(um2(i-1)-dv1);
    n_fitm2(i) = n_fitm2(i-1)+d_n*dt;
    
    if tvm2(idx-1) <= tm2(i-1)
        T = cTn*n_fitm2(i-1)*abs(n_fitm2(i-1))-cTnv*v_fitm2(idx-1)*abs(n_fitm2(i-1))-cTv*v_fitm2(idx-1)*abs(v_fitm2(idx-1));
...
for i = 2:length(n_fitm3)
    Q = cQn*n_fitm3(i-1)*abs(n_fitm3(i-1))-cQnv*v_fitm3(idx-1)*abs(n_fitm3(i-1))-cQv*v_fitm3(idx-1)*abs(v_fitm3(idx-1));
    d_n = -kn*n_fitm3(i-1)-kq*Q+kv1*(um3(i-1)-dv1);
    n_fitm3(i) = n_fitm3(i-1)+d_n*dt;
    
    if tvm3(idx-1) <= tm3(i-1)
        T = cTn*n_fitm3(i-1)*abs(n_fitm3(i-1))-cTnv*v_fitm3(idx-1)*abs(n_fitm3(i-1))-cTv*v_fitm3(idx-1)*abs(v_fitm3(idx-1));
