...
for i = 1:length(Td)
    ...
    % Control update
    nd(i) = (cTnv*vhat(i)+sign(Td(i))*sqrt((cTnv*vhat(i))^2+abs(4*cTn*(cTv*vhat(i)*abs(vhat(i))+Td(i)))))/(2*cTn);
    ndot_d = (nd(i)-nhat(i))/dt;
    cdot = nhat(i) - nd(i);
    c = c + cdot*dt;
    a = ndot_d+kn*nd(i)+kq*(cQn*nd(i)*abs(nd(i))-cQnv*vhat(i)*abs(nd(i))-cQv*vhat(i)*abs(vhat(i)));
    ...
    Q = cQn*nhat(i)*abs(nhat(i))-cQnv*vhat(i)*abs(nhat(i))-cQv*vhat(i)*abs(vhat(i));
    ...
    xdot = f+K*(y(:,i)-yhat)+[0;(.1+kV+.1*abs(nhat(i)))*(y(2,i)-vahat(i));(.6+.1*abs(nhat(i)))*(y(2,i)-vahat(i))];
    nhat(i+1) = nhat(i)+xdot(1)*dt;
    vhat(i+1) = vhat(i)+xdot(2)*dt;
    vahat(i+1) = vahat(i)+xdot(3)*dt;
    A = [-kn-2*kq*cQn*abs(nhat(i))+kq*cQnv*vhat(i)*sign(nhat(i))                                                   kq*cQnv*abs(nhat(i))+2*kq*cQv*abs(vhat(i))                       0
             2*kt*cTn*abs(nhat(i))-kt*cTnv*vhat(i)*sign(nhat(i)) -kv-2*kvv*abs(vhat(i))+kvv*vahat(i)*sign(vhat(i))-kt*cTnv*abs(nhat(i))-2*kt*cTv*abs(vhat(i))        kvv*abs(vhat(i))
           2*kTV*cTn*abs(nhat(i))-kTV*cTnv*vhat(i)*sign(nhat(i))                                                -kTV*cTnv*abs(nhat(i))-2*kTV*cTv*abs(vhat(i)) -kV-2*kVV*abs(vahat(i))];
